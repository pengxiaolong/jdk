...
// (_alloc_regions points to a heap-allocated, cache-line-padded array of _alloc_region_count
// Atomic<ShenandoahHeapRegion*> slots); a thread bumps a region's atomic top lock-free. All three
// template instantiations (mutator, collector, old-collector) share an identical layout, so a
// single SA mirror reads any of them.
public class ShenandoahPartitionAllocator extends VMObject {
    private static CIntegerField allocRegionCountField;
    private static AddressField  allocRegionsField;
    private static long          allocRegionSlotSize;
    ...
    private static synchronized void initialize(TypeDataBase db) {
        ...
        // _alloc_regions points to a heap-allocated PaddedArray of ShenandoahPaddedAllocRegion.
        allocRegionsField = type.getAddressField("_alloc_regions");
        // Each slot is cache-line padded to avoid false sharing, so its stride is the padded
        // element size, not a bare pointer.
        allocRegionSlotSize = db.lookupType("ShenandoahPaddedAllocRegion").getSize();
    }
    ...
    public long remnantBytes() {
        ...
        Address base = allocRegionsField.getValue(addr);
        if (base == null) {
            return 0;
        }
        for (long i = 0; i < count; i++) {
            Address regionAddr = base.getAddressAt(i * allocRegionSlotSize);
            if (regionAddr == null) {
                continue;
            }
