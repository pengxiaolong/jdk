...
void ShenandoahHeap::reset_bytes_allocated_since_gc_start() {
  ...
  size_t mutator_remaining_bytes = _free_set->mutator_allocator()->remaining_bytes();
  if (mode()->is_generational()) {
    unaccounted_bytes = young_generation()->heuristics()->force_alloc_rate_sample(bytes_allocated - mutator_remaining_bytes);
  } else {
    // Single-gen Shenandoah uses global heuristics.
    unaccounted_bytes = heuristics()->force_alloc_rate_sample(bytes_allocated - mutator_remaining_bytes);
  }
  _free_set->reset_bytes_allocated_since_gc_start(unaccounted_bytes + mutator_remaining_bytes);
}
