  ...
  for (;;) {
    ...
    if (free_words >= size) {
      if (try_allocate(obj /*value*/, size, obj /*reference*/)) {
        adjust_alloc_metadata(req, size);
        ready_for_replenish = (free_words - size) < plab_min_size_words();
        return obj;
      }
      if (obj == nullptr) {
      ...
      ready_for_replenish = free_words < plab_min_size_words();
      return nullptr;
    }
  ...
  for (;;) {
    ...
    if (adjusted_size >= min_size) {
      if (try_allocate(obj /*value*/, adjusted_size, obj /*reference*/)) {
        ...
        ready_for_replenish = free_words - adjusted_size < plab_min_size_words();
        return obj;
      }
