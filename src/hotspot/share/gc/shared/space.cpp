...
inline HeapWord* ContiguousSpace::par_allocate_impl(size_t size, uint &attempts) {
  do {
    HeapWord* obj = top();
    if (pointer_delta(end(), obj) >= size) {
      HeapWord* new_top = obj + size;
...
// Lock-free.
HeapWord* ContiguousSpace::par_allocate(size_t size, uint &max_attempts) {
